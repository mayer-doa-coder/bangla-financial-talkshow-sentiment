...
\def\aw{27mm}
\node[fbox=\aw, fill=cAsk, draw=cAskB, minimum height=23mm] (a1)
  {\fhd{Domain problem / stakeholder need}
   Which panellist holds which position on the economy?};
\node[fbox=\aw, fill=cSig, draw=cSigB, minimum height=23mm, right=6mm of a1] (a2)
  {\fhd{Data acquisition / source}
   Bangla financial television talk shows, as broadcast audio};
\node[fbox=\aw, fill=cDat, draw=cDatB, minimum height=23mm, right=6mm of a2] (a3)
  {\fhd{Curated \& documented dataset}
   Speaker turns, and one verdict per speaker per episode};
\node[fbox=\aw, fill=cMod, draw=cModB, minimum height=23mm, right=6mm of a3] (a4)
  {\fhd{ML / analytics use}
   Turn sentiment, and the speaker's position across an episode};
\node[fbox=\aw, fill=cOut, draw=cOutB, minimum height=23mm, right=6mm of a4] (a5)
  {\fhd{Decision, service or practical impact}
   Who is negative on the economy, and whether that is changing};
...
{\sffamily\small\setstretch{1.0}\textbf{Figure 1(a).} Practical application framework. The
intended users are financial-media monitors, macroeconomic policy analysts and Bangla NLP
researchers. The practical outcome is a stance signal resolved to the individual speaker, which
a document-level sentiment label cannot produce because a panel programme contains people who
...
\begin{tikzpicture}[node distance=0mm]
\def\pw{33mm}
% ---- row 1: from recordings to a leakage-safe split
\node[fbox=\pw, fill=cSig, draw=cSigB, minimum height=17mm] (p1)
  {\fhd{Raw data} Broadcast audio and its episode registry};
\node[fbox=\pw, fill=cSig, draw=cSigB, minimum height=17mm, right=7mm of p1] (p2)
  {\fhd{Quality checks} Duplicate, duration and audio hygiene screening};
\node[fbox=\pw, fill=cSig, draw=cSigB, minimum height=17mm, right=7mm of p2] (p3)
  {\fhd{Preprocessing} Who spoke when, speech to text, then words joined into
   speaker turns};
\node[fbox=\pw, fill=cDat, draw=cDatB, minimum height=17mm, right=7mm of p3] (p4)
  {\fhd{Train / validation / test split} Grouped by episode, so no episode
   falls on two sides};
% ---- row 2: from representation to a read-out
\node[fbox=\pw, fill=cDat, draw=cDatB, minimum height=17mm, below=19mm of p1] (p5)
  {\fhd{Features / representation} Speaker role with turn text, and the
   speaker's full script};
\node[fbox=\pw, fill=cMod, draw=cModB, minimum height=17mm, right=7mm of p5] (p6)
  {\fhd{ML model(s)} A pretrained Bangla encoder, fine-tuned, with pooling for
   the speaker level};
\node[fbox=\pw, fill=cMod, draw=cModB, minimum height=17mm, right=7mm of p6] (p7)
  {\fhd{Evaluation metrics} Macro-F1 and per-class scores, read against
   baselines};
\node[fbox=\pw, fill=cOut, draw=cOutB, minimum height=17mm, right=7mm of p7] (p8)
  {\fhd{Interpretation / application} A stance read-out per speaker, with its
   limits stated};
\foreach \i/\j in {p1/p2, p2/p3, p3/p4, p5/p6, p6/p7, p7/p8} \draw[farr] (\i) -- (\j);
% the second row continues from the end of the first
\draw[farr] (p4.south) -- ++(0,-9.5mm) -| (p5.north);
\end{tikzpicture}
\end{center}

{\sffamily\small\setstretch{1.0}\textbf{Figure 1(b).} Technical pipeline followed in the course
project, shown at stage level. Splitting is grouped by episode rather than by row, so turns from
one panel cannot fall on both sides of the split, and the duplicate episode is removed before
partitioning. Section 6 gives the same pipeline with the models, settings and counts actually
used.\par}
...
{\sffamily\small\setstretch{1.0}\textbf{Figure 2.} The same pipeline as Figure~1(b), now with the models, settings and
counts actually used. Splitting is grouped by episode, never by row, so that turns from one
panel cannot appear on both sides of the split; the deduplicated episode is removed before
partitioning. All 45 episodes carry trusted \texttt{pyannote} diarization in this build.\par}
...
\vspace{2pt}
Four properties decide whether a resource can answer the question in Figure~1(a). It has to be
