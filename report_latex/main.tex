...
& \thd{Repository}      & Not yet public --- offline archive submitted to the course instructor \\ \hline
\end{tabularx}
\end{center}
...
\def\bw{26.5mm}
\node[fbox=\bw, fill=cAsk, draw=cAskB, minimum height=17mm] (a1)
  {\fhd{Analyst question} Who \emph{in the panel} is negative on the economy?};
\node[fbox=\bw, fill=cSig, draw=cSigB, minimum height=17mm, right=7mm of a1] (a2)
  {\fhd{Broadcast input} 45 talk-show episodes, 28.6\,h, 3 channels};
\node[fbox=\bw, fill=cDat, draw=cDatB, minimum height=17mm, right=7mm of a2] (a3)
  {\fhd{This dataset} 2{,}414 speaker turns; 1{,}630 supervised labels};
\node[fbox=\bw, fill=cMod, draw=cModB, minimum height=17mm, right=7mm of a3] (a4)
  {\fhd{Stance model} BanglaBERT turn classifier $+$ speaker aggregation};
\node[fbox=\bw, fill=cOut, draw=cOutB, minimum height=17mm, right=7mm of a4] (a5)
  {\fhd{Deployed signal} Speaker-resolved sentiment index over air-time};
\foreach \i/\j in {a1/a2, a2/a3, a3/a4, a4/a5} \draw[farr] (\i) -- (\j);
\foreach \n/\k in {a1/1, a2/2, a3/3, a4/4, a5/5}
  \node[fnum] at ($(\n.north west)+(1.6mm,-1.6mm)$) {\k};
\node[bandlbl, below=1.2mm of a1] {NEED};
\node[bandlbl, below=1.2mm of a3] {ARTEFACT};
\node[bandlbl, below=1.2mm of a5] {USE};
\end{tikzpicture}
\end{center}

{\sffamily\small\setstretch{1.0}\textbf{Figure 1(a).} Practical application framework. Intended
users are financial-media analysts, macroeconomic policy monitors and Bangla NLP researchers; the
practical outcome is a speaker-resolved stance signal over broadcast financial discourse that
document-level sentiment cannot produce.\par}
\vspace{6pt}
...
{\sffamily\small\setstretch{1.0}\textbf{Figure 1(b).} Technical pipeline followed in the course
...
\vspace{8pt}
\begin{center}
\fbox{\parbox{0.95\textwidth}{\sffamily\small\textbf{FIRST-PAGE RULE:} Keep Page 1 as a visual
synopsis. Use concise labels; detailed explanations belong in the main report.}}
\end{center}
...
\thd{Corresponding student} & Sarwad Hasan Siddiqui (2107006), \underline{\hspace{2.2cm}}@stud.kuet.ac.bd \\ \hline
...
\thd{Repository / persistent ID} & Not publicly released at submission; the dataset and code are submitted as an offline archive to the course instructor. A DOI should be minted at public release. \\ \hline
...
TF--IDF reference of 0.471, and a hierarchical speaker model scores 0.530 macro-F1 in grouped
...
\thd{Subject / domain}          & Computer science --- natural language processing; applied economics / financial media analysis \\ \hline
...
\thd{Dataset size}              & 45 unique episodes ($\approx$28.6 h audio); 5{,}395 ASR decode windows; \textbf{2{,}414 speaker turns} ($\approx$219{,}000 words); 212 speaker--episode units; 150 judged units; 1{,}811 turn labels; 1{,}630 supervised turns \\ \hline
...
\thd{Accessibility}             & Not publicly released at submission; the dataset and code are submitted as an offline archive to the course instructor. A DOI should be minted at public release. Derived annotations, transcripts and timings are included in the archive; source audio is not. \\ \hline
...
\begin{itemize}[leftmargin=1.4em,itemsep=2pt,topsep=2pt]
  ...
  \item Every intermediate artefact is preserved --- diarization RTTM, per-word ASR probabilities,
        fusion alignments and judge rationales with verbatim evidence quotes --- so the corpus can
...
Bangladesh's recent macroeconomic period --- banking-sector stress, foreign-reserve pressure and
sustained inflation --- was debated nightly on television business programmes. These broadcasts
...
resources label \emph{utterances} or \emph{documents}, not \emph{speakers}. As Figure 1(a) shows,
...
recovered from the audio and then aligned to ASR words. The pipeline in Figure 1(b) is the
...
\textbf{Re-judging after the second pipeline run.} When the bundles were re-processed --- twenty
additional episodes, and \texttt{pyannote} re-run over roll \texttt{2107009} --- the turn
...
\begin{itemize}[leftmargin=1.4em,itemsep=3pt,topsep=2pt]
  ...
  rejected as unjudgeable and never sent --- untrusted diarization, unassigned speaker, or fewer
  clean words than the human-set minimum --- leaving 150. A further three were withdrawn from the
...
(\texttt{annotation\_drafts/}, 42 files, 4{,}886 utterances) were prepared for exactly that step
...
\texttt{turn\_id}      & Unique turn identifier & string & --- & \texttt{roll::ep::}\newline\texttt{turnNNNN} & never null \\ \hline
\texttt{global\_episode} & Episode grouping key used for splitting & string & --- & \texttt{roll::epNNN} & never null \\ \hline
...
\texttt{text}          & Normalised Bangla transcript of the turn & string & --- & free text & \texttt{""} if empty \\ \hline
\texttt{n\_words}      & Whitespace word count of \texttt{text} & integer & $\ge 0$ & --- & never null \\ \hline
\texttt{asr\_confidence} & Mean log-probability over the turn's words & float & $\le 0$ & --- & null if no words \\ \hline
\texttt{diarization\_trusted} & False for the five fallback-diarized episodes & boolean & --- & \texttt{true}/\texttt{false} & never null \\ \hline
\texttt{role}          & Inferred conversational role & string & --- & \texttt{host}, \texttt{guest}, \texttt{minor}, \texttt{unassigned} & \texttt{unassigned} \\ \hline
\texttt{label}         & Judge turn label (labelled files only) & string & --- & \texttt{negative}, \texttt{neutral}, \texttt{positive} & row dropped if \texttt{illegible} \\ \hline
\texttt{judge\_confidence} & Judge's stated confidence in the turn label & float & $[0,1]$ & --- & never null \\ \hline
\texttt{evidence}      & Verbatim quote supporting the label & string & --- & substring of \texttt{text} & \texttt{""} \\ \hline
\end{longtable}
\endgroup
...
ASR decode windows & 5{,}395 \\ \hline
...
(mean 146 words) and carry the evaluative content. Table~1 gives the breakdown.
...
evidence the rubric was applied consistently across both judging rounds.}
...
\begin{itemize}[leftmargin=1.4em,itemsep=3pt,topsep=2pt]
  ...
  \item \textbf{Window $\rightarrow$ turn reconstruction.} The ``utterances'' in the fused files
        ...
        the 5{,}395 having a duration of 27.2--28.0 s. Every window boundary truncates a word, so
  ...
  \item \textbf{Leakage control.} The grouping key is \texttt{global\_episode}. Splits are
        ...
        the presenter's phrasing and the same ASR error signature --- a random row split would let
...
\begin{longtable}{|L{3.2cm}|L{2.6cm}|L{1.8cm}|L{2.4cm}|L{4.2cm}|}
\hline
\thd{Dataset} & \thd{Domain / modality} & \thd{Language} & \thd{Unit labelled} & \thd{Relation to this dataset} \\ \hline
\endfirsthead
\hline \thd{Dataset} & \thd{Domain} & \thd{Language} & \thd{Unit} & \thd{Relation} \\ \hline \endhead
BLUB / BanglaBERT benchmark \cite{banglabert} & Mixed written text & Bangla & Sentence / document & Source of the backbone encoder; written-text domain, no speaker structure \\ \hline
BLP-2023 Task 2 \cite{blp2023} & Social media (written) & Bangla & Post & Closest Bangla sentiment benchmark; different genre, no speaker attribution \\ \hline
BAN-ABSA \cite{banabsa} / BABSA \cite{babsa} & News and reviews (written) & Bangla & Aspect--sentence & Aspect-based; this dataset's 52-target lexicon enables an analogous speaker $\times$ target extension \\ \hline
MELD / IEMOCAP (ERC) \cite{erc} & TV / acted dialogue, multimodal & English & Utterance & Conversational and multimodal, but utterance-level; no aggregated speaker verdict \\ \hline
Earnings-call transcript corpora \cite{earnings} & Financial speech (transcribed) & English & Document / speaker & Nearest analogue in argument; English, professionally transcribed, not ASR-noisy \\ \hline
\textbf{This dataset} & \textbf{TV talk show, audio + ASR} & \textbf{Bangla} & \textbf{Speaker turn + speaker--episode} & --- \\ \hline
\end{longtable}
...
\begin{itemize}[leftmargin=1.4em,itemsep=2pt,topsep=2pt]
  \item \textbf{Dual-level supervision.} Turn labels and a holistic speaker verdict are produced in
        one judging call over the speaker's full script, so the two levels are coherent by
        construction; measured bottom-up/top-down agreement is 85.0\% exact and 87.1\% directional.
  \item \textbf{Speaker--episode as a released unit.} Existing conversational sentiment corpora stop
        at the utterance; this release ships the aggregated speaker unit with a continuous stance
...
Two linked supervised tasks. \textbf{Primary:} turn-level 3-class sentiment classification ---
input $X$ is the pair (\texttt{role}, \texttt{turn text}), target $y \in$ \{\texttt{negative},
\texttt{neutral}, \texttt{positive}\}, unit of prediction is one speaker turn. \textbf{Secondary:}
speaker-level stance --- predicted turn labels for one speaker--episode unit are aggregated into a
verdict in \{\texttt{negative}, \texttt{mixed}, \texttt{neutral}, \texttt{positive}\} plus a
continuous stance score. The practical output is a speaker-resolved stance signal per episode.
...
classes and a workable share of positive turns in every partition. Table~2 gives the partition.
...
\thd{TF--IDF + logistic regression} & Character 3--5-grams (\texttt{char\_wb}, \texttt{min\_df}=2, sublinear TF, 40{,}771 features), balanced class weights, $C$ selected on the validation split from $\{0.5,1,2,4,8\}$. The transparent non-neural reference. \\ \hline
...
Majority class            & ---    & 0.236 & 0.549 & 0.000 & Floor \\ \hline
TF--IDF char 3--5 + LogReg & 0.572 & 0.471 & 0.620 & 0.329 & Non-neural reference; $C=0.5$ \\ \hline
...
(0.704) and then declines while training loss keeps falling --- the model begins to overfit 1{,}009
training turns, and the epoch-4 checkpoint is the one restored and evaluated.}
\end{figure}
...
Guest & 184 & --- & 0.696 & 0.610 \\ \hline
Host  & 140 & --- & 0.707 & 0.468 \\ \hline
...
\subsection{Speaker-level results}

The speaker verdict is the unit the dataset exists for. Two routes were evaluated: aggregating the
predicted turn labels with a fixed rule (Section 10.7, Table~6), and a hierarchical model that
learns the aggregation (Table~7).
...
it would be selection on the test set.}
...
report quotes; the 30-unit held-out split in Table~6 is a sanity check, not a measurement.}
\begin{tabular}{|l|c|c|c|r|}
\hline
...
the turn level a standard Bangla encoder reaches macro-F1 0.670 against a 0.236 floor and a 0.471
non-neural reference, from 1{,}009 training turns under an episode-disjoint split, with
$\kappa=0.496$ --- moderate agreement with the judge on a three-class problem. At the speaker level,
...
\begin{itemize}[leftmargin=1.4em,itemsep=3pt,topsep=2pt]
  ...
  \item \textbf{Environment.} Python 3.10+, \texttt{torch} 2.12, \texttt{transformers},
        ...
        \texttt{csebuetnlp} normalizer --- BanglaBERT was pretrained on normalised text and the
  ...
  \item \textbf{Precautions.} Filter to \texttt{diarization\_trusted = true} for any claim about
        ...
        Do not tune on the speaker-level validation split --- it has six units and two of four
        classes are thin; use \texttt{GroupKFold} over all 147 valid units instead.
  ...
  \item \textbf{Inappropriate uses.} Do not present model output as the identified opinion of a
        named individual --- speaker IDs are clustering predictions, not identities. Do not quote
...
(53\%), partly because the period genuinely was --- a banking crisis, an energy shock and
...
pass. Human effort went into curation rather than labelling --- rubric design, diarization repair,
unit screening, out-of-domain exclusion and the discarding of stale blocks, all itemised with
counts in Section~5.5 --- but \textbf{no human annotator re-labelled individual turns}, and the
annotation drafts in the roll folders still record \texttt{human\_verified: false} for all 4{,}886
...
\thd{Repository name}                   & None yet --- offline archive submitted to the course instructor; Zenodo is the intended host at public release \\ \hline
\thd{Dataset DOI / persistent identifier} & Not assigned (see above) \\ \hline
\thd{Direct dataset URL}                & Not assigned (see above) \\ \hline
...
\thd{Code / notebook URL}               & Included in the same archive under \texttt{speaker\_sentiment/}. Contains \texttt{build\_speaker\_corpus.py}, \texttt{llm\_judge.py}, \texttt{make\_datasets.py}, \texttt{predict\_speakers.py} and \texttt{Speaker\_Sentiment\_BanglaBERT.ipynb}. \\ \hline
...
episode availability was uneven --- some programmes published far more usable broadcasts than
others. \textbf{Overall share} is the team's agreed division of the whole workload, which includes
...
\emph{Data curation; methodology; validation; investigation; writing --- review \& editing.}
...
editing.} Collection and pipeline run for roll 2107004; the corpus-build, judge-merge and
...
\emph{Validation; data curation; investigation; writing --- review \& editing.} Collection and
...
\emph{Software; formal analysis; visualization; data curation; writing --- review \& editing.}
...
\thd{Findable}       & Repository and persistent identifier/URL are provided; file/folder names are meaningful; metadata are searchable. & $\square$~Yes~/~$\boxtimes$~No & No DOI or public URL at submission; file and folder names are meaningful and \texttt{registry.csv} is searchable \\ \hline
...
\thd{Raw + processed separation} & Original observations are preserved separately from cleaned/derived data. & $\boxtimes$~Yes~/~$\square$~No & Audio in roll folders; derived artefacts in \texttt{results/} \\ \hline
