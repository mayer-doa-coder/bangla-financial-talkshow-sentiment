...
- The complete submission-gate table, including honest SKIPPED status for
  WER, DER, sentiment F1, and error propagation when gold references are absent.
...
```powershell
python demo/audit_demo.py --data-root datasets --expected-episodes 30 `
  --report datasets/demo_readiness.json
```

This is a structural readiness check, not a substitute for WER, DER, sentiment
F1, or human validation. It fails if an episode, audio source, transcript, or
required speaker-attribution coverage is missing.
...
Open `http://127.0.0.1:7860`. To create a temporary public link:

```powershell
python demo/app.py --data-root datasets --share
```
